...
class RyseBLEDeviceConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    async def _async_pair(self, service_info: BluetoothServiceInfoBleak) -> str | None:
        """Bond with the device, then release the connection.

        Returns an error key, or None on success. Pairing is refused unless the
        latest advertisement still has the PAIR flag set; BlueZ reports
        AuthenticationFailed if the shade is idle.
        """
    ...
    async def async_step_user(
        ...
        # user holds its PAIR button, which is also how we recognise a RYSE device.
